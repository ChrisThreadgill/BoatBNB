...
function EditBoatForm({ boat, setShowModal }) {
  ...
  const [year, setYear] = useState(boat.year);
  ...
  const [value, setValue] = useState(boat.address);
  ...
  const handleSubmit = async () => {
    ...
    if (!errs && address) {
      ...
      dispatch(boatsActions.updateOneBoat(boat)).then((boat) => {
        setBoatSuccess(true);
        setDisabled(true);
        setTimeout(async () => {
          setShowModal(false);
  ...
  return (
    <div className="boat__edit__form__container">
      <div className="boat__edit__header">Edit your listing's details</div>
